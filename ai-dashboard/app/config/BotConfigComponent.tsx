...
export default function BotConfigComponent({ initialData }: { initialData: any }) {
  ...
  const [previewMessages, setPreviewMessages] = useState<any[]>([
    { role: "assistant", content: greeting || "Hello! Drop a message to test my live configurations." }
  ]);
  ...
  // 💬 Handle Sandbox Chat Execution Stream
  const handleSendPreviewMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPreviewMessages(prev => [...prev, { role: "assistant", content: "Error communicating with local neural model node." }]);
      }
    } catch (err) {
      setPreviewMessages(prev => [...prev, { role: "assistant", content: "Pipeline failure connection broken." }]);
    } finally {
 ...
 const handleUpdate = async () => {
  ...
  window.FusionAIChatConfig = { tenantSlug: "${initialData?.slug || "workspace"}" };
</script>
                ...
                <div className="flex-1 p-4 overflow-y-auto space-y-3 font-sans text-xs bg-black/20">
                  ...
                  {isBotTyping && (
                    <div className="flex justify-start">
                      <div style={{ color: textColor }} className="bg-white/[0.04] rounded-xl px-4 py-3 font-mono text-[10px] tracking-widest opacity-40 animate-pulse">
                        PROCESSING MATRIX...
                      </div>
                    </div>
                  )}
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          <ConfigCard icon={<Building2 size={14} />} title="Tenant Registry Overview">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
              <AccountField icon={<Building2 size={16} />} label="Registered Company Name" value={initialData?.name || "Fusion Space"} />
              <AccountField icon={<Terminal size={16} />} label="Isolation Domain Slug Identifier" value={`/${initialData?.slug || "unknown"}`} />
            </div>
          </ConfigCard>
        </div>
      )}
...
function ConfigCard({ icon, title, children }: any) {
  return (
    <div className="bg-zinc-900/10 border border-white/5 rounded-[28px] p-6 lg:p-8 backdrop-blur-md hover:border-white/10 transition-all duration-300 focus-within:border-[#d4ff33]/20">
      <div className="flex items-center gap-2.5 mb-5 text-zinc-400">
        <span className="text-[#d4ff33]">{icon}</span>
        <h3 className="font-black uppercase tracking-[0.22em] text-[10px] text-zinc-400">
          {title}
        </h3>
      </div>
      {children}
